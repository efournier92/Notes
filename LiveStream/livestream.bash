...
if [[ "$mode" = 'advanced' ]]; then
  ...
  ffmpeg -re \
    ...
    -filter_complex '[0:v]scale=ih*16/9:-1,boxblur=luma_radius=min(h\,w)/20:luma_power=1:chroma_radius=min(cw\,ch)/20:chroma_power=1[bg];[0:v]crop=700:540:260:0[cropped];[bg][cropped]overlay=0:(H-h)/2,crop=h=iw*9/16[withbg];[1:v]scale=iw/1.6:ih/1.6[pip];[withbg][pip]overlay=main_w-overlay_w-0:(H-h)/2[vid]' -map [vid] \
